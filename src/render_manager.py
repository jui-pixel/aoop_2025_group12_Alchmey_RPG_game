...
class RenderManager:
    ...
    def draw_weapon_selection(self) -> None:
        ...
        if not self.game.storage_manager.weapons_library:
            error_text = font.render("No weapons available!", True, (255, 0, 0))
            self.screen.blit(error_text, (SCREEN_WIDTH // 2 - error_text.get_width() // 2, 100))
        else:
            for i, weapon in enumerate(self.game.storage_manager.weapons_library):
                count = self.game.event_manager.selected_weapons.count(weapon)
                is_selected = weapon in self.game.event_manager.selected_weapons
                mouse_pos = pygame.mouse.get_pos()
                hovered = False
        ...
        count_text = font.render(f"Selected: {len(self.game.event_manager.selected_weapons)}/{max_weapons}", True, (255, 255, 255))
        self.screen.blit(count_text, (SCREEN_WIDTH // 2 - count_text.get_width() // 2, 400))
        pygame.display.flip()
